...
A default `llm` dispatcher MAY read its system prompt from one of three sources. The inline `system_prompt` field, or its `prompt` alias, carries the prompt text directly. The `system_prompt_file` field names a manifest relative path that a file based loader resolves against the manifest root using the same rule as a rego `bundle` path, and the dispatcher reads the file at evaluation time. The `system_prompt_url` field is an object with a `url` member and a `sha256` or `integrity` pin that the dispatcher fetches over the extends fetch path and trust gate defined in section 2.2. At most one of these sources may be set, and a manifest that sets more than one MUST fail closed with `runtime_error:manifest_invalid`. A `system_prompt_url` MUST use HTTPS and MUST declare a pin, so an unpinned or non HTTPS prompt URL MUST fail closed. A pinned fetch whose bytes do not match the pin, a fetch error, a body size breach, or a missing file MUST fail closed as an annotator error. When no source is set the dispatcher uses its preset default prompt.
